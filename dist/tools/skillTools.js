 ...
 */
export async function getSkill(projectPath, skillId) {
 ...
 * Create a new skill
 */
export async function createSkill(projectPath, skillData) {
    const skills = await getSkills(projectPath);
 ...
 * Delete a skill
 */
export async function deleteSkill(projectPath, skillId) {
    const skills = await getSkills(projectPath);
    const skillIndex = skills.findIndex(skill => skill && skill.id === skillId);
    if (skillIndex === -1) {
        return false;
 ...
 */
export async function searchSkills(projectPath, searchTerm) {
    const skills = await getSkills(projectPath);
    const lowerSearchTerm = searchTerm.toLowerCase();
    return skills.filter(skill => skill && (skill.name.toLowerCase().includes(lowerSearchTerm) ||
        skill.description.toLowerCase().includes(lowerSearchTerm)));
}
/**
 * Create a damage skill (attack spell or physical skill)
 */
export async function createDamageSkill(projectPath, name, damageFormula, mpCost, scope, elementId, description) {
 ...
 */
export async function createHealingSkill(projectPath, name, healFormula, mpCost, scope, description) {
    return await createSkill(projectPath, {
        name,
 ...
 * Create a buff skill
 */
export async function createBuffSkill(projectPath, name, buffType, turns, mpCost, scope, description) {
    return await createSkill(projectPath, {
        name,
 ...
 */
export async function createDebuffSkill(projectPath, name, debuffType, turns, mpCost, scope, description) {
    return await createSkill(projectPath, {
        name,
 ...
 */
export async function createStateSkill(projectPath, name, stateId, chance, mpCost, scope, description) {
    return await createSkill(projectPath, {
        name,
